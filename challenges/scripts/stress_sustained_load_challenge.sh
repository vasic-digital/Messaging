...
echo "=== Messaging Stress Sustained-Load Challenge ==="
echo "  url=$HEALTH_URL dur=${DURATION}s rps=${RPS}"

if [[ -z "$HEALTH_URL" ]]; then
    echo "[1/6] SKIP: MESSAGING_HEALTH_URL unset — SKIP-OK: #env-no-target"
    echo "=== Messaging Stress Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== Messaging Stress Challenge: PASSED ==="
