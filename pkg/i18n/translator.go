...
// Messaging submodule. It is the CONST-046-compliant indirection between
...
// default), and the Messaging submodule never reaches into a parent project
// to discover its own catalogue.
//
// Usage:
//
//	t := i18n.NoopTranslator{} // production default — returns key verbatim
//	msg := t.T("messaging_broker_connection_failed", map[string]any{
//	    "broker": "rabbitmq",
//	})
//
// Consuming projects wire a real translator that loads
// bundles/active.en.yaml + locale overrides; the Messaging submodule
// remains project-not-aware.
//
// Round-123 §11.4 — kickoff scope: a curated sentinel set of broker error
// keys + a handful of validation/publish/consumer call sites. Subsequent
// rounds widen coverage to the full broker/producer/consumer/kafka/rabbitmq
// surface.
package i18n
...
// error/log surface shape ("messaging_broker_connection_failed")
